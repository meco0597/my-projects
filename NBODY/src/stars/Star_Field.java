	...
	public void add_planets_to_solar_system( int count )
	{
		...
		else
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				// makes the bounds of rectangle that surrounds the sun for the mouseclick
				if(planets.get(i) instanceof Star){
					bounds = new Rectangle2D.Double(planets.get(i).getX(),planets.get(i).getY(), planets.get(i).getWidth(), planets.get(i).getHeight());
				}
			}
		}
	...
	public void mouseDragged( MouseEvent e )
	{
		...
		for (int i = 0; i < comps.length; i++)
		{
			if (comps[i] instanceof Satellite)
			{
				...
				// renews the bounds for the rectangle
				if(sat instanceof Star){
					bounds = new Rectangle2D.Double(sat.getX(),sat.getY(), sat.getWidth(), sat.getHeight());
				}
			}
		}
	}
		...
		else
		{
			...
			for (int i = 0; i < comps.length; i++)
			{
				if (comps[i] instanceof Satellite)
				{
					...
					// renews the bounds of the rectangle
					if(sat instanceof Star){
						bounds = new Rectangle2D.Double(sat.getX(),sat.getY(), sat.getWidth(), sat.getHeight());
					}
				}
